...
public class SnowflakesView : UIView {
    ...
    func addNewSnowflake() {
        ...
        let snowflake = Snowflake(position: snowflakePosition, minimumSize: 4, maximumSize: 8)
    ...
    func startSnowflakeLifecycle(snowflake: UIView) {
        ...
        let utfgt = UInt64(Int64(Double(arc4random_uniform(100)+500)/100 * Double(NSEC_PER_SEC)))
        
        let deadlineTime =  DispatchTime.init(uptimeNanoseconds: utfgt+DispatchTime.now().uptimeNanoseconds)
...
//        dispatch_after(dispatch_time(dispatch_time_t(DispatchTime.now()), Int64(Double(arc4random_uniform(100)+500)/100 * Double(NSEC_PER_SEC))), DispatchQueue.main) {
//            // Remove the snowflake.
//            self.gravityBehaviour1.removeItem(snowflake)
//            self.gravityBehaviour2.removeItem(snowflake)
//            snowflake.removeFromSuperview()
